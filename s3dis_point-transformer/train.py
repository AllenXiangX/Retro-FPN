...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/s3dis/s3dis_retrofpn.yaml', help='config file')
    parser.add_argument('opts', help='see config/s3dis/s3dis_retrofpn.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def get_logger(save_path):
    ...
    handler = logging.FileHandler(filename=os.path.join(save_path, "train.log"))
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (coord, feat, target, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        if args.arch == "retro_fpn":
            output, x2, x3, x4, x5, idx2, idx3, idx4, idx5 = model([coord, feat, offset], train=True)
            target1 = target
            ...
            loss2 = criterion(x2, target2)
            loss3 = criterion(x3, target3)
            loss4 = criterion(x4, target4)
            loss5 = criterion(x5, target5)
