...
class ScanNet3D(data.Dataset):
    ...
    def __init__(self, dataPathPrefix='Data', voxelSize=0.05,
        ...
        print('split:', split)
        if split == 'trainval':
            self.data_paths = sorted(glob(join(dataPathPrefix, 'train', '*.pth')) + glob(join(dataPathPrefix, 'val', '*.pth')))
        else:
        ...
        if memCacheInit and (not exists("/dev/shm/wbhu_scannet_3d_%s_%s_locs_%08d" % (split, self.identifier, 0))):
            ...
            for i, (locs, feats, labels) in enumerate(torch.utils.data.DataLoader(
                ...
                feats = (feats + 1.) * 127.5
                sa_create("shm://wbhu_scannet_3d_%s_%s_locs_%08d" % (self.split, self.identifier, i), locs)
                sa_create("shm://wbhu_scannet_3d_%s_%s_feats_%08d" % (self.split, self.identifier, i), feats)
                sa_create("shm://wbhu_scannet_3d_%s_%s_labels_%08d" % (self.split, self.identifier, i), labels)

        print('[*] %s (%s) loading done (%d)! ' % (dataPathPrefix, self.split, len(self.data_paths)))

    def __getitem__(self, index_long):
        index = index_long % len(self.data_paths)
        locs_in = SA.attach("shm://wbhu_scannet_3d_%s_%s_locs_%08d" % (self.split, self.identifier, index)).copy()
        feats_in = SA.attach("shm://wbhu_scannet_3d_%s_%s_feats_%08d" % (self.split, self.identifier, index)).copy()
        labels_in = SA.attach("shm://wbhu_scannet_3d_%s_%s_labels_%08d" % (self.split, self.identifier, index)).copy()
        labels_in[labels_in == 255] = 20  # for voxel sampling only
        ...
        # while coords.shape[0] < 300000:
            # n_e = 300000 - coords.shape[0]
            # coords = torch.cat([coords, coords[:n_e, :]], 0)

            # feats = torch.cat([feats, feats[:n_e, :]], 0)

            # labels = torch.cat([labels, labels[:n_e]], 0)
