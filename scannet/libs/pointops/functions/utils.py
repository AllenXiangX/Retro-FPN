...
def query_and_group2(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True, return_idx=False, subtract=True):
    """
    ...
    if new_xyz is None:
        new_xyz = xyz
    if idx is None:
        idx, _ = knn_query(nsample, xyz, offset, new_xyz, new_offset)  # (m, nsample)
