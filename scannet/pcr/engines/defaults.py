...
color_map = np.array([  # for debugging labels
    (174., 199., 232.),
    (152., 223., 138.),
    (31., 119., 180.),
    (255., 187., 120.),
...
def write_pointcloud(filename,xyz_points,rgb_points=None, axis=0):
...
class Trainer:
    ...
    def run_step(self, i, input_dict):
        ...
        with torch.cuda.amp.autocast(enabled=self.cfg.enable_amp):
            ...
            if self.cfg.model.type == 'retrofpn': # Retro-FPN
                label1 = input_dict["label"].clone()
                label1[label1 == 255] = 20  
                label_mat = label2Matrix(label1, 21).cuda().float()  # 20+1
                label_mat.require_grad = False
                input_dict['label_mat'] = label_mat

                outputs, f2, f3, f4, f5, label2, label3, label4, label5 = self.model(input_dict, train=True)

                label = input_dict["label"]
                label2[label2==20] = 255
                label3[label3 == 20] = 255
                label4[label4 == 20] = 255
                label5[label5 == 20] = 255

                loss1 = self.criteria(outputs, label)
                loss2 = self.criteria(f2, label2)
                loss3 = self.criteria(f3, label3)
                loss4 = self.criteria(f4, label4)
                loss5 = self.criteria(f5, label5)
                loss = loss1 + loss2 + loss3 + loss4 + loss5 # retro

                
            else:
                output = self.model(input_dict) #  ptv2
                loss = self.criteria(output, input_dict["label"])  # ptv2
